...
export function EmendaTableView({ emendas, onRowClick }: EmendaTableViewProps) {
  ...
  const paginatedEmendas = useMemo(() => {
    ...
    >
      <div className="flex items-center space-x-1">
        <span>{children}</span>
        {sortField === field && (
          <svg className={`w-3 h-3 transform ${sortDirection === 'desc' ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-[1000px] w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <SortableHeader field="ano" className="w-12">
                Ano
              </SortableHeader>
              <SortableHeader field="acao" className="w-64">
                Ação / Projeto
              </SortableHeader>
              <SortableHeader field="dotacao" className="w-20 text-right">
                Dotação
              </SortableHeader>
              <SortableHeader field="ministerio" className="w-20">
                Ministério
              </SortableHeader>
              <SortableHeader field="autor" className="w-40">
                Autor
              </SortableHeader>
              <SortableHeader field="rp" className="w-12">
                RP
              </SortableHeader>
              <SortableHeader field="modalidade" className="w-24">
                Modalidade
              </SortableHeader>
              <SortableHeader field="relacionamento" className="w-32">
                Relacionamento
              </SortableHeader>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {paginatedAndSortedEmendas.items.map((emenda, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-2 whitespace-nowrap text-center">
                    <button 
                      className="text-[#003366] hover:text-[#004080] text-xs font-medium transition-colors inline-flex items-center justify-center hover:bg-blue-50 px-1 py-1 rounded w-full"
                      onClick={(e) => {
                        e.stopPropagation();
                        onRowClick(emenda);
                      }}
      ...
      {/* Paginação da tabela */}
      {paginatedAndSortedEmendas.totalPages > 1 && (
        <div className="bg-gray-50 border-t border-gray-200 px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-700">
              Mostrando {currentPage * ITEMS_PER_PAGE + 1} - {Math.min((currentPage + 1) * ITEMS_PER_PAGE, paginatedAndSortedEmendas.totalItems)} de {paginatedAndSortedEmendas.totalItems} emendas
            </div>
            <div className="flex space-x-2">
              ...
              <div className="flex space-x-1">
                {Array.from({ length: paginatedAndSortedEmendas.totalPages }, (_, i) => {
                  // Mostrar apenas algumas páginas próximas à atual
                  const shouldShow = paginatedAndSortedEmendas.totalPages <= 7 || 
                                   i === 0 || 
                                   i === paginatedAndSortedEmendas.totalPages - 1 ||
                  ...
                  if (!shouldShow) {
                    ...
                    if (i === currentPage + 3 && currentPage < paginatedAndSortedEmendas.totalPages - 4) {
                      return <span key={`ellipsis-${i}`} className="px-2 py-1 text-xs text-gray-500">...</span>;
                    }
                    return null;
                  }
              ...
              <button
                onClick={() => setCurrentPage(Math.min(paginatedAndSortedEmendas.totalPages - 1, currentPage + 1))}
                disabled={currentPage === paginatedAndSortedEmendas.totalPages - 1}
                className="px-3 py-1 text-xs bg-white border border-gray-300 rounded-md hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Próximo
              </button>
